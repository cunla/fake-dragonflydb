...
[fake-dragonflydb Code of Conduct](https://github.com/cunla/fake-dragonflydb/blob/master/CODE_OF_CONDUCT.md).
...
This section guides you through submitting an enhancement suggestion for fake-dragonflydb, **including completely new features
...
- **Explain why this enhancement would be useful** to most fake-dragonflydb users. You may also want to point out the other
...
- Note: In order to run the tests, a real redis server should be running.
  The tests are comparing the results of each command between fake-dragonflydb and a real redis.
    - You can use `docker-compose up redis6` or `docker-compose up redis7` to run redis.
- Run test with coverage using `poetry run pytest -v --cov=fake-dragonflydb --cov-branch`
